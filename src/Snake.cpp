...
void Snake::Extend() {
    ...
    // There's two cases here essentially either there's only 1 segment or there's more than 1 segment.
    // Depending on this we determine where to place the new segment.
...
void Snake::Render(sf::RenderWindow &l_window) {
    ...
    m_bodyRect.setFillColor(sf::Color(202, 217, 152));
    m_bodyRect.setPosition(head->position.x * m_size, head->position.y * m_size);
    l_window.draw(m_bodyRect);

    m_bodyRect.setFillColor(sf::Color(128, 191, 140));
    for (auto it = m_snakeBody.begin() + 1; it != m_snakeBody.end(); ++it) {
